...
import com.hexa.muinus.store.dto.store.StoreRegisterDTO;
...
public class StoreFlimarketValidatorImpl implements ConstraintValidator<StoreFlimarketValidator, Object> {
    ...
    private boolean validateFlimarketFields(StoreRegisterDTO registerDTO, ConstraintValidatorContext context) {
        ...
        if (registerDTO.getFlimarketYn() == YesNo.Y) {
            ...
            validateFliImage(imageValid, context);
            validateFliSectionCnt(sectionValid, context);
    ...
    private void validateFliImage(boolean imageValid, ConstraintValidatorContext context) {
        if (!imageValid) {
    ...
    private void validateFliSectionCnt(boolean sectionValid, ConstraintValidatorContext context) {
        if (!sectionValid) {
            context.disableDefaultConstraintViolation();
